...
export default function HouseholdClient() {
  ...
  const updateRole = async (user_id: string, role: string) => {
    ...
    try {
      ...
      toast({ title: "Couldn’t update role", description: e?.message ?? "Please try again." });
      setStatusLine("Couldn’t save.");
    }
  };

  const requestRemove = (user_id: string) => {
    setDeleteConfirm({ open: true, user_id, label: "Remove this member?" });
  };
          ...
          <CardContent className="space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              {!active ? null : editingName ? (
                <div className="flex items-center gap-2">
                  <Chip onClick={() => void saveRename()}>Save</Chip>
            ...
            {!active ? (
              ...
              <div className="space-y-1">
                ...
                <div className="text-xs text-zinc-500">Household ID: {active.id}</div>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3">
            ...
            {members.length === 0 ? (
              ...
              <div className="grid gap-2">
                {members.map((m) => (
                  <div key={m.user_id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-zinc-200 px-3 py-2">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-zinc-900">{m.user_id}</div>
                    ...
                    {allowMemberEdits ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Chip onClick={() => requestRemove(m.user_id)}>Remove</Chip>
                      </div>
                    ) : (
